...
class Tool(object):
    ...
    def taskAudit(self,parent_card_no,type=3):
        """
        任务结算财务审核
        :return:
        """

        parent_card_no =parent_card_no
    ...
    def test_task(self,task_id,parent_card_no):
        ...
        # V端用户登录
        res_v = SetupApi().login_v("13729543993", 'e10adc3949ba59abbe56e057f20f883e')
        SaveValue.TOKEN_V = res_v['access_token']
        SaveValue.V_UUID = res_v['uuid']

        # C端用户登录
        res_c = SetupApi().login_c("13729543836", 'e10adc3949ba59abbe56e057f20f883e')
        SaveValue.TOKEN_C = res_c['access_token']
        SaveValue.C_UUID = res_c['uuid']

        param = {'task_id': task_id}
        param2 = {'parent_card_no': parent_card_no}
        type = 3
        print(Tool().task_apply(type=type, **param))
        print(Tool().task_subaudit(type=type, **param))
        if type == 2 or type == 3:
            # 填写侧品地址
            print(Tool().task_ceaddress(type=type, **param))
            # 发货
            print(Tool().task_carrier(type=type, **param))
            # 确认收获啊
            print(Tool().task_getcarrier(type=type, **param))
        # 开团地址
        print(Tool().task_addurl(type=type, **param))
        # 验收
        print(Tool().task_check(type=type, **param2))
...
if __name__ == '__main__':
    ...
    param = {'task_id':177773}
    param2 = {'parent_card_no':'20-10185100-136'}
    type = 3
    print(Tool().task_apply(type=type,**param))
    print(Tool().task_subaudit(type=type,**param))
    if type == 2 or type == 3:
    ...
    print(Tool().task_check(type=type,**param2))
    #审核
    # print(Tool().taskAudit(type=type,parent_card_no=param2['parent_card_no']))
# 177264 177263  177262
