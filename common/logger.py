...
log_path = os.path.join(os.path.dirname(cur_path), 'logs/autotest/api')
...
class Log():
    def __init__(self):
        ...
        if path == 'run_production.py': # 运行线上
            self.logname = os.path.join(log_path, '%s.log' % ('apiproduction_' + time.strftime('%Y_%m_%d')))
        else:
